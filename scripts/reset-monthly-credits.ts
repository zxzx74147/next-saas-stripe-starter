  ...
  for (const user of users) {
    try {
      ...
      if (!subscriptionConfig) {
        console.warn(`No subscription config found for tier: ${tier}, skipping user ${user.id}`);
        continue;
      }
      ...
      if (user.creditBalance.overageThisMonth > 0) {
        console.log(`User ${user.id} has overage charges: $${user.creditBalance.overageThisMonth.toFixed(2)}`);
        usersWithOverage++;
