...
export async function calculateCreditCost(
  ...
  const effectsMultiplier = params.hasAdvancedEffects ? EFFECTS_MULTIPLIER : 1.0;
...
export async function validateCreditBalance(
  ...
  if (!features.videoGeneration) {
    return {
      ...
      creditBalance: 0,
      requiredCredits: 0,
      reason: 'Your current plan does not include video generation. Please upgrade to a paid plan.'
    };
  }
  ...
  const { totalCredits, estimatedCostUSD } = await calculateCreditCost(params, userId);
 ...
 */
export async function getSubscriptionConfigForUser(user: Pick<User, 'stripePriceId'>): Promise<any> {
...
export async function getEstimatedCreditUsage(userId: string): Promise<{
  ...
  const percentUsed = monthlyAllocation > 0 ? (used / monthlyAllocation) * 100 : 0;
