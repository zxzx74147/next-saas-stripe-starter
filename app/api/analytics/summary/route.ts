...
export async function GET(req: NextRequest) {
  try {
    ...
    const videoMetrics = videos.map(video => {
      ...
      return {
        id: video.id,
        title: (video.videoSettings as any).prompt || `Video ${video.id.substring(0, 8)}`,
        views,
        shares,
        embeds,
        downloads,
    ...
    const totalShares = videoMetrics.reduce((sum, video) => sum + video.shares, 0);
    const totalEmbeds = videoMetrics.reduce((sum, video) => sum + video.embeds, 0);
    const totalDownloads = videoMetrics.reduce((sum, video) => sum + video.downloads, 0);
    
    // Find most viewed and most shared videos
    const sortedByViews = [...videoMetrics].sort((a, b) => b.views - a.views);
    const sortedByShares = [...videoMetrics].sort((a, b) => b.shares - a.shares);
    
    const mostViewedVideo = sortedByViews.length > 0 ? {
    ...
    const mostSharedVideo = sortedByShares.length > 0 ? {
